...
def get_Faces(output_size, n_epoch, batch_size):
    images_path = tl.files.load_file_list(path=flags.data_dir, regx='.*.png', keep_prefix=True, printable=False)
    ...
    def _map_fn(image_path):
        ...
        image = image[128:896, 0:768, :]
        image = tf.image.resize([image], (output_size, output_size))[0]
        return image

    dataset = tf.data.Dataset.from_generator(generator_train, output_types=tf.string)
